...
async def startup_db_client():
    await db.connect_db()
    # Seed initial scam patterns if collection is empty
    count = await db.scam_patterns.count_documents({})
    if count == 0:
        seed_patterns = [
            {"pattern_type": "phrase", "pattern_value": "pay registration fee", "severity": "high", "description": "Common job scam phrase"},
            {"pattern_type": "phrase", "pattern_value": "telegram @", "severity": "medium", "description": "Uses Telegram for recruitment"},
            {"pattern_type": "phrase", "pattern_value": "easy money fast", "severity": "critical", "description": "Unrealistic earnings claim"},
            {"pattern_type": "phrase", "pattern_value": "click here for easy money", "severity": "critical", "description": "Phishing clickbait"}
        ]
        await db.scam_patterns.insert_many(seed_patterns)
        print("✓ Seeded initial scam patterns into MongoDB")
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "mongodb": "connected" if db.client else "disconnected"}

@app.post("/feedback")
async def receive_feedback(request: FeedbackRequest):
    """Store user feedback regarding analysis results."""
    success = await memory_service.save_feedback(request.dict())
    if not success:
...
async def analyze_enhanced(
    ...
    try:
        ...
        result = await analyzer.analyze_with_company(job_request, company_url)
        return result
    except Exception as e:
        safe_error = str(e).encode('ascii', 'replace').decode('ascii')
