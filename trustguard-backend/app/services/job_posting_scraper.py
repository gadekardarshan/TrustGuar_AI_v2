...
class JobPostingScraper:
    ...
    def _extract_job_description(self, content: str, url: str) -> str:
        ...
        if len(description) < 100:
            # Fallback: return original content but truncated
            description = content
    ...
    def _extract_company_name(self, content: str, metadata: Dict, url: str) -> str:
        ...
        if metadata.get("title"):
            ...
            # Indeed/Naukri format: "Job Title - Company Name"
            elif " - " in title:
                parts = title.split(" - ")
                if len(parts) > 1:
                    candidate = parts[1].strip()
                    # Validate candidate is not generic
                    if candidate.lower() not in ["jobs", "careers", "hiring", "vacancy", "position", "employment", "recruitment", "work"]:
