...
class Analyzer:
    ...
    async def analyze(self, request: AnalyzeRequest) -> AnalyzeResponse:
        # 1. MongoDB Scam Pattern Pre-check (Hybrid Memory Layer)
        pattern_matches, pattern_boost = await memory_service.get_pattern_boost(request.text or "")
        ...
        if request.url:
            ...
            domain_reputation_score = domain_result["score_deduction"] 
            domain_reasons = domain_result["reasons"]

        # 3. Rules Engine Analysis
        rules_data = self.rules_engine.analyze(request.text or "")
        
        # 4. LLM Semantic Analysis (Hybrid AI Layer)
        # Skip AI if pattern boost is already critical? (Optional Optimization)
        ...
        score_result = scoring_engine.combine(
            rules_data, 
            llm_data, 
        ...
        # Add matched patterns to reasons
        for match in pattern_matches:
            all_reasons.append(f"Matched known scam pattern: {match['pattern']}")
    ...
    async def analyze_with_company(self, job_data: AnalyzeRequest, company_url: Optional[str] = None) -> EnhancedAnalyzeResponse:
        """
        ...
        job_analysis = await self.analyze(job_data)
